...
let s:tlist_debug_file = '/tlist.dbg'
...
function! s:Tlist_Refresh()
    ...
    if tlist_win != -1
        ...
        "let save_winnr = winnr()
        let save_winnr = s:mwinnr()
        call s:Tlist_Log_Msg("save_winnr".save_winnr)
...
function! s:mwinnr()
    let start = winnr()
            call s:Tlist_Log_Msg("start_mwinnr:".start)
        if &previewwindow
            call s:Tlist_Log_Msg("In Preview")
          if exists("w:pwinnr") != 0
            call s:Tlist_Log_Msg("mwinnr:".w:pwinnr)
            return w:pwinnr
          endif
        endif
    wincmd w
